...
const Fitness = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:5000/api/fitnesses/create", userData,{headers: {Authorization: `Bearer ${user.token}`}});
    ...
    return (
      ...
      >
        ...
        <input
          ...
          onChange={(e) => setUserData({ ...userData, exercises: e.target.value })}
        />
        <input
          type="tel"
        ...
        <input
          ...
          className="w-full p-3 mb-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          onChange={(e) => setUserData({ ...userData, distance: e.target.value })}
        />
        <button
          type="submit"
